...
void findEyes(cv::Mat frame_gray, cv::Rect face) {
    ...
    //-- Find Eye Centers
    cv::Point2d leftPupil = findEyeCenter(faceROI,leftEyeRegion,"Left Eye");
    cv::Point2d rightPupil = findEyeCenter(faceROI,rightEyeRegion,"Right Eye");
    // get corner regions
...
void detectAndDisplay( cv::Mat frame ) {
    ...
    cv::Mat filtred_gray_frame;
    cv::medianBlur(frame_gray, filtred_gray_frame, 3);
    
    //cvtColor( frame, frame_gray, CV_BGR2GRAY );
    //equalizeHist( frame_gray, frame_gray );
    //cv::pow(frame_gray, CV_64F, frame_gray);
    //-- Detect faces
    face_cascade.detectMultiScale( filtred_gray_frame, faces, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE|CV_HAAR_FIND_BIGGEST_OBJECT, cv::Size(150, 150) );
    //  findSkin(debugImage);
    ...
    //-- Show what you got
    if (faces.size() > 0) {
        findEyes(filtred_gray_frame, faces[0]);
    }
}
